# Spec: Watchdog (QA-Entity) — v0.3

**Status:** Draft v0.3
**Datum:** 2026-04-27
**Vorgaenger:** `spec-qa-entity-v0.2.md`
**Pre-Mortem:** `brain/pre-mortem-watchdog-v0.2-20260427.md`

## Aenderungen ggue. v0.2

Aus dem Pre-Mortem auf v0.2 abgeleitet:

- **Single-Writer auf Testplan-File** — explizite Anforderung an die Integrations-Session, nicht "irgendwie live-syncen". Race-Conditions waren der haerteste Pre-Mortem-Befund (Score 16).
- **Phase-Gate vor Phase 1 als sichtbare Empfehlung mit Begruendung** statt Heuristik-Sprung im Black Box. Watchdog sagt was er denkt und warum, User entscheidet.
- **Persona-Default-Marker** — eigener Abschnitt definiert was aus Relay uebernommen wird und was *nicht*. Die weichzeichnende Hoeflichkeit bleibt draussen. Do/Don't-Beispiele zurueck aus v0.1.
- **Mindest-Aktivierungs-Wege als Anforderung** — drei Trigger (Voice, MCP-Tool, Sidebar/Preset), nicht einer. (b) ist Pflicht, weil ohne automatischen Trigger aus dem Workflow Watchdog ungenutzt bleibt.
- **Plan-Luecken kriegen einen eigenen Output-Kanal** — Note mit Tag `plan-luecke`, zusaetzlich zur Abschluss-Meldung. Damit findbar via Tag-Suche, unabhaengig davon ob jemand die Abschluss-Meldung liest.
- **Annahmen-Section** — Mitschnitt-Verwerfen und Free-Form-Uferlosigkeit als bewusst akzeptierte Risiken benannt, nicht beseitigt.
- **STT-Toleranz** als eigene Guardrail — Voice-Input transkribiert nicht fehlerfrei, Watchdog behandelt das nicht als Bug, fragt aber nach wenn es bei wichtigen Details ins Raten kippen wuerde.
- **Schreiben fuer den Empfaenger** als eigene Guardrail — Watchdog ist Schreib-Job, nicht Reflektions-Job. Keine User-Paraphrasen, keine Gespraechs-Zusammenfassungen, sondern direkt verwertbare Eintraege fuer Entwicklungs-/Fix-Session.
- **Bug-Recherche vor Bugreport-Filing** — bei nicht-trivialen Bugs schaut Watchdog kurz ob es sich um einen bekannten Vorfall der eingesetzten Technologie handelt (Whisper-Halluzinationen, Electron-Eigenheiten, tmux-Output-Glitches, better-sqlite3 ABI). Treffer und Workarounds wandern als eigene Sektion in den Bugreport.
...
3. **Plan-Luecken-Note** (wenn welche aufgetreten sind) als eigenstaendige Note mit Tag `plan-luecke` und Verweis auf den betroffenen Testplan. Eine Note pro Lauf wenn noetig, sammelt alle Luecken dieses Laufs. Damit ist die Information per Tag-Suche auffindbar, ohne in der Abschluss-Meldung versickern zu koennen.
4. **Abschluss-Meldung:** Kurzer md-Block — "X/Y bestanden, Z Bugs, K Feature-Requests, Bugreport-IDs: ..., Plan-Luecken-Note: ...". Wer den Block empfaengt, klaert die Integrations-Session.
...
User entscheidet. Watchdog folgt. Damit ist die Heuristik aus dem Black Box geholt — der User sieht was Watchdog rechnet und kann widersprechen.

**Default-Empfehlungen (Watchdog nutzt sie als Ausgangspunkt seiner Begruendung):**
...
1. Watchdog stellt vor was er verstanden hat — knapp, in der Form die spaeter ins File wandert (nicht als User-Paraphrase, siehe Guardrail "Schreiben fuer den Empfaenger"):
   > "Drei Bugs, ein Feature-Request, vier Testcases als implizit bestanden markiert. Details: ..."
2. User korrigiert / bestaetigt / streicht
3. Konsens wird ins Testplan-File geschrieben — direkt in Empfaenger-Form (Status-Aenderungen bei abgehakten Cases, Bug-Kurzbeschreibung mit Repro-Schritten, Feature-Request als Anforderung formuliert, ggf. Anker-Verlinkung auf den spaeter gefilten Bugreport). Keine Vorgeschichte, keine "User sagte..."-Konstruktionen.
...
Bei trivialen Bugs entfaellt die Sektion oder reduziert sich auf eine Zeile "Recherche: nicht erforderlich (trivialer UI-Bug)". Watchdog spekuliert nicht — wenn die Recherche keinen klaren Treffer ergibt, steht das so drin. Auch das ist verwertbar: die Fix-Session weiss, dass die offensichtlichen Quellen schon geprueft sind.
...
4. Abschluss-Meldung formuliert: "X/Y bestanden, Z Bugs, K Feature-Requests, Bugreport-IDs: ..., Plan-Luecken-Note: ..."
...
### A2 — Mindest-Aktivierungs-Wege

Watchdog muss ueber drei Wege aktivierbar sein:

**(a) Voice-Command** — z.B. "Watchdog starten" via existierende Voice-Pipeline.

**(b) MCP-Tool** — Pflicht. Der MPO oder Orchestrator muss Watchdog programmatisch nach Worker-Completion triggern koennen, ohne dass der User manuell aktivieren muss. Ohne (b) bleibt Watchdog dem User immer hinterher — der vergisst die Aktivierung, der MPO entscheidet selbst weiter, Bugs bleiben in der Pipeline haengen.
...
Was die Folge-Session konkret klaeren und bauen muss (ueber A1-A3 hinaus):
...
4. **Phase 2 (~5-10 min):** Watchdog: "Aus dem freien Reden: 1 Bug (Verzoegerung beim Preset-Start ~2s), 1 Unklarheit (zwei mal 'absenden' schnell hintereinander — Verhalten unklar), implizit bestanden: T-LC.2, T-LC.3, T-LC.6, T-K.3, T-K.4, T-K.5. Stimmt das fuer dich?" — User bestaetigt mit Korrektur. Watchdog traegt ein, verwirft den Rohtext.

5. **Phase 3 (~30-60 min):** Verbleibende ~42 Cases clustered durchgehen. Pro Cluster: Status setzen, bei FAILs Bug-Recherche (~1-5 Min je nach Fund) und Bugreport (4 Stueck). Beispiel: BR-1235 betrifft Whisper-Halluzination "verwendet." bei Stille — Recherche findet bekanntes Issue im whisper.cpp Repo, Workaround per VAD-Threshold dokumentiert. Geht so in den Bugreport.

6. **Phase 4 (~5 min):** Plan-Luecken-Note geschrieben (Tag `plan-luecke`, eine Luecke aus Phase 0). Abschluss-Meldung: "44/50 bestanden, 4 Bugs (BR-1234 bis BR-1237), 1 Feature-Request, 2 BLOCKED (Voice-Modell nicht installiert). Plan-Luecken-Note: <Note-ID>."
