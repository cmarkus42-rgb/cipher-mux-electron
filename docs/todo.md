...
| # | Task | Status | Depends | Parallel | Dateien | Akzeptanzkriterien |
|---|------|--------|---------|----------|---------|--------------------|
| 5.1 | **Orchestrator-Setup** — Verzeichnis erstellen (~/.config/cipher-mux/orchestrator/), CLAUDE.md aus Template generieren (MCP-Config-Injection), Orchestrator als spezielle Session starten | done (2026-04-14) | #3.2, #2.3 | — | src/main/session/orchestrator-template.ts, session-manager.ts (erweitern) (~2 Dateien) | Orchestrator-Verzeichnis + CLAUDE.md existiert, Session startet mit MCP-Config |
| 5.2 | **Orchestrator-UI** — Blitz-Icon in Activity Rail, Start/Stop Toggle, Orchestrator-Messages im Chatroom hervorheben | done (2026-04-14) | #5.1, #3.4, #2.6 | — | ActivityRail.tsx (erweitern), ChatroomPanel.tsx (erweitern) (~2 Dateien) | Blitz-Icon sichtbar, Klick startet Orchestrator, Messages visuell unterscheidbar |
| 5.3 | **KickoffManager** — Projektverzeichnis erstellen, Requirements-Datei kopieren, Scaffold vorbereiten (CLAUDE.md, docs/), tmux-Session spawnen, /interview auto-starten | done (2026-04-14) | #2.3, #4.1 | #5.1 | src/main/project/kickoff-manager.ts (~1 Datei) | Kickoff erstellt Verzeichnis, kopiert Requirements, startet Session mit Interview |
| 5.4 | **Kickoff-Dialog UI** — KickoffDialog.tsx: Dateipfad-Input (mit Electron file picker), Zielverzeichnis-Input, Projektname, Bestätigung. Cmd+N trigger. | done (2026-04-14) | #5.3, #1.4 | — | src/renderer/components/KickoffDialog.tsx (~1 Datei) | Cmd+N öffnet Dialog, File-Picker funktioniert, Bestätigung triggert Kickoff |
| 5.5 | **MCP Auto-Injection** — Bei jedem Session-Start: MCP-Server-Config (.claude/settings.json oder Env-Vars) automatisch in die Claude Code Session injizieren | done (2026-04-14) | #3.2, #2.3 | #5.1 | src/main/session/session-manager.ts (erweitern) (~1 Datei) | Neue Sessions haben automatisch MCP-Tools verfügbar |
...
| # | Task | Status | Depends | Parallel | Dateien | Akzeptanzkriterien |
|---|------|--------|---------|----------|---------|--------------------|
| 6.1 | **Session-Recovery** — Bei App-Start: tmux-Sessions erkennen, bekannte Sessions wiederherstellen, unbekannte als orphaned anzeigen, UI zum Beenden von Orphans | done (2026-04-17) | #2.3, #2.6 | — | session-manager.ts (recover, adoptOrphan, killOrphan), RecoveryDialog.tsx (~2 Dateien) | Nach Crash+Neustart: Sessions wieder sichtbar, Orphans beendbar |
| 6.2 | **Layout-Persistenz** — Grid-Layout bei jeder Änderung speichern (debounced 500ms), bei App-Start wiederherstellen | done (2026-04-17) | #2.5, #1.5 | #6.1 | useGrid.ts (persist/restore), config-store.ts (~2 Dateien) | Layout überlebt App-Neustart |
| 6.3 | **Info-Seite** — InfoSettingsView.tsx: Shortcuts-Tabelle, Feature-Wiki, Settings-Tab (Scan-Pfade). Activity Rail Icon. | done (2026-04-17) | #2.6, #1.4 | #6.1 | src/renderer/components/InfoSettingsView.tsx (~1 Datei) | Erreichbar via Activity Rail, zeigt Shortcuts, Features und Settings |
| 6.4 | **Bugreport-Modus** — BugreportManager (Main): Diagnostik + Ollama-Enrichment. BugreportDialog.tsx (Renderer): Text + Screenshot + Voice-Interview. Markdown-Export in Outbox. | done (2026-04-17) | #2.3, #1.5 | #6.3 | src/main/bugreport/bugreport-manager.ts, src/renderer/components/BugreportDialog.tsx (~2 Dateien) | Bugreport sammelt Diagnostik, Ollama-Enrichment, Export als Markdown |
| 6.5 | **Keyboard-Shortcuts komplett** — ShortcutRegistry + useShortcuts Hook existieren, Cmd+Alt+I (DevTools) via globalShortcut. Übrige Shortcuts (Cmd+B etc.) noch nicht verdrahtet. | done (2026-04-17) | #2.6, #5.4, #3.4 | — | src/renderer/shortcut-registry.ts, hooks/useShortcuts.ts, main.ts (~3 Dateien) | Registry + Hook implementiert, Verdrahtung teilweise offen |
| 6.6 | **Integration & Smoke-Test** — TESTCASE.md mit manuellen E2E-Tests, 164 Unit-Tests grün, Build sauber | done (2026-04-17) | #5.4, #6.1, #6.5 | — | docs/TESTCASE.md, test/main/*.test.ts (~20+ Dateien) | Tests grün, manuelle Testfälle dokumentiert |
